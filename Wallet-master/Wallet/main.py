...
wallets = read_wallets()  # собираем все кошельки в коллекцию.
history = list()  # собираем историю.
...
while True:
    ...
    if command == 'BLOCK_WALLET':
        ...
        print(f'Операция выполнена успешно. Состояние номера счета {wallet.owner_id}: {"заблокирован" if wallet.is_blocked else "разблокирован"}.\n\n')
        continue
    ...
    if command == 'MONEY_OUT':
        ...
        history.append(HistoryItem(wallet.owner_id, command, delta_balance, wallet.currency, '', wallet.balance))
    ...
    if command == 'TRANSACT':
        ...
        history.append(HistoryItem(wallet_sender.owner_id, command, -delta_balance, wallet_sender.currency, wallet_recipient.owner_id, wallet_sender.balance))
        history.append(HistoryItem(wallet_recipient.owner_id, command, diff_delta_balance, wallet_recipient.currency, wallet_sender.owner_id, wallet_recipient.balance))
        continue

    if command == 'SHOW_HISTORY':
        for i in range(len(history)):
            print(f'Операция: {i+1} {history[i].owner_id, history[i].type_of_operation, history[i].delta_balance, history[i].currency, history[i].recipient_id, history[i].owner_new_balance}. \n')
        continue
